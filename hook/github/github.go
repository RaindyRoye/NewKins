...
func Parse(req *http.Request, secret string) (wb hook.WebHook, err error) {
	defer func() {
		if r := recover(); r != nil {
			...
			if e, ok := r.(error); ok {
				...
				err = fmt.Errorf("github: panic in Parse: %w", fmt.Errorf("%v", r))
			}
		}
	}()
	data, err := io.ReadAll(
	...
	default:
		return nil, fmt.Errorf("hook contains unknown header: %w", fmt.Errorf("%s", req.Header.Get(hook.GithubEvent)))
	}
	if err != nil {
