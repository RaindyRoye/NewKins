...
func UpMysqlMigrate(ul string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("%w: %w", ErrPingDatabase, err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("%w: %w", ErrInitDriver, err)
	}
	...
	if err != nil {
		return fmt.Errorf("%w: %w", ErrInitSource, err)
	}
	...
	mgt, err := migrate.NewWithInstance(
		...
		"mysql", driver)
	if err != nil {
		return fmt.Errorf("%w: %w", ErrCreateMigrateInstance, err)
	}
	defer func() { _, _ = mgt.Close() }()
	err = mgt.Up()
	if err != nil && !errors.Is(err, migrate.ErrNoChange) {
		_ = mgt.Down()
		return fmt.Errorf("%w: %w", ErrRunMigration, err)
	}
