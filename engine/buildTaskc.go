...
func (c *BuildTask) genRunjob(stage *runtime.Stage, job *jobSync) (rterr error) {
	...
	for i, v := range runjb.Commands {
		...
		_, err = comm.Db.Context(c.taskCtx()).InsertOne(cmd)
		if err != nil {
			if _, delErr := comm.Db.Context(c.taskCtx()).Where("build_id=? and step_id=?", cmd.BuildId, cmd.StepId).Delete(cmd); delErr != nil {
...
func (c *BuildTask) gencmds(runjb *runners.RunJob, cmds []any) (rterr error) {
	defer util.RecoverResult(&rterr, "BuildTask.gencmds")
	for _, v := range cmds {
		switch val := v.(type) {
		case string:
