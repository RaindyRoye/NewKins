...
func (c *baseRunner) ReadDir(fs int, buildID string, pth string) ([]*runners.DirEntry, error) {
	if buildID == "" || pth == "" {
		return nil, fmt.Errorf("readDir: %w (buildID=%q, path=%q)", ErrEmptyParameter, buildID, pth)
	}
	build, ok := Mgr.buildEgn.Get(buildID)
	if !ok {
...
func (c *baseRunner) ReadFile(fs int, buildID string, pth string, start int64) (int64, io.ReadCloser, error) {
	if buildID == "" || pth == "" {
		return 0, nil, fmt.Errorf("readFile: %w (buildID=%q, path=%q)", ErrEmptyParameter, buildID, pth)
	}
	build, ok := Mgr.buildEgn.Get(buildID)
	if !ok {
	...
	if pths == "" {
		return 0, nil, fmt.Errorf("readFile: %w, no path resolved", ErrInvalidFilesystem)
	}
...
func (c *baseRunner) GenEnv(buildID, jobId string, env utils.EnvVal) error {
	if jobId == "" || env == nil {
		return fmt.Errorf("genEnv: %w (jobId=%q, env is nil: %v)", ErrEmptyParameter, jobId, env == nil)
	}
	tsk, ok := Mgr.buildEgn.Get(buildID)
	if !ok {
...
func (c *baseRunner) StatFile(fs int, buildID, jobId string, dir, pth string) (*runners.FileStat, error) {
	if jobId == "" || pth == "" {
		return nil, fmt.Errorf("statFile: %w (jobId=%q, path=%q)", ErrEmptyParameter, jobId, pth)
	}
	build, ok := Mgr.buildEgn.Get(buildID)
	if !ok {
	...
	if pths == "" {
		return nil, fmt.Errorf("statFile: %w, no path resolved", ErrInvalidFilesystem)
	}
	stat, err := os.Stat(pths)
	if err != nil {
...
func (c *baseRunner) UploadFile(fs int, buildID, jobId string, dir, pth string, start int64) (io.WriteCloser, error) {
	if jobId == "" || pth == "" {
		return nil, fmt.Errorf("uploadFile: %w (jobId=%q, path=%q)", ErrEmptyParameter, jobId, pth)
	}
	build, ok := Mgr.buildEgn.Get(buildID)
	if !ok {
	...
	if pths == "" {
		return nil, fmt.Errorf("uploadFile: %w, no path resolved", ErrInvalidFilesystem)
	}
...
func (c *baseRunner) FindArtVersionId(buildID, idnt string, names string) (string, error) {
	...
	if buildID == "" || idnt == "" || name == "" {
		return "", fmt.Errorf("findArtVersionId: %w (buildID=%q, identifier=%q, name=%q)", ErrEmptyParameter, buildID, idnt, name)
	}
	build, ok := Mgr.buildEgn.Get(buildID)
	if !ok {
	...
	if !perm.CanExec() {
		return "", fmt.Errorf("user %q: %w for artifactory %q", pv.Uid, ErrPermissionDenied, idnt)
	}
	...
	if err != nil {
		...
		return "", fmt.Errorf("%w %q", ErrArtifactNotFound, names)
	}
	...
	if err != nil {
		...
		return "", fmt.Errorf("%w %q (with version)", ErrArtifactNotFound, names)
	}
	return artv.Id, nil
}
func (c *baseRunner) NewArtVersionId(buildID, idnt string, name string) (string, error) {
	...
	if buildID == "" || idnt == "" || name == "" {
		return "", fmt.Errorf("newArtVersionId: %w (buildID=%q, identifier=%q, name=%q)", ErrEmptyParameter, buildID, idnt, name)
	}
	build, ok := Mgr.buildEgn.Get(buildID)
	if !ok {
